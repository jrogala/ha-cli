...
// NewClient creates an authenticated Home Assistant client from config.
func NewClient() (*client.Client, error) {
	token := config.Token()
	if token == "" {
		return nil, fmt.Errorf("no token configured, run 'ha setup --token TOKEN' or set HA_TOKEN")
	}
	return client.New(config.URL(), token), nil
}
...
// NewTabWriter returns a new tabwriter for aligned output.
